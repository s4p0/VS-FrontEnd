...
class Device(Base, DomainObject):
	...
	videos = relationship('Video', backref='device')
...
class Video(Base, DomainObject):
	...
	optionsCompassEnabled = Column('options_compass_enabled', Boolean)
	# options video
	optionsVideoQuality = Column('options_video_quality', Float)
	#
	geom = GeometryColumn(Point(2))
	unique_device = relationship("Device", backref=backref('video', order_by=id))
	deviceid = Column('device', Unicode, ForeignKey('devices.uuid'))

class Audio(Base, DomainObject):
	__tablename__ = 'audios'
	# file
	id = Column('audio_id',Integer, primary_key=True)
	fileName = Column('file_name',Unicode)
	filePath = Column('file_path',Unicode)
	created = Column(DateTime, default=datetime.now())
	# compass
	...
	#
	geom = GeometryColumn(Point(2))
	unique_device = relationship("Device", backref=backref('audio', order_by=id))
	deviceid = Column('device', Unicode, ForeignKey('devices.uuid'))
...
def pictures():
	...
	print ('pictures returned "%r"') % len(results)
	# pdb.set_trace()
...
def videos():
	...
	if bbox is not '':
		...
		box = 'Polygon((%s %s, %s %s, %s %s, %s %s, %s %s))' % (xmin, ymin, xmin, ymax, xmax, ymax, xmax, ymin, xmin,ymin)
		# pdb.set_trace()	
		vids = session.query(Video).filter(Video.geom.within(box))
	else:
		print 'no bbox'
	...
	result = {}
	result['result'] = results
	print ('videos returned "%r"') % len(results)
	# pdb.set_trace()
	return Response(json.dumps(result), mimetype='application/json')
...
def audios():
	...
	if bbox is not '':
		...
		box = 'Polygon((%s %s, %s %s, %s %s, %s %s, %s %s))' % (xmin, ymin, xmin, ymax, xmax, ymax, xmax, ymin, xmin,ymin)
		# pdb.set_trace()	
		auds = session.query(Audio).filter(Audio.geom.within(box))
	else:
		auds = session.query(Audio).all()
...
def mobile_upload():
	...
	GeometryDDL(Video.__table__)
	GeometryDDL(Audio.__table__)
...
@app.route("/api/csv_fixed_data", methods=['POST', 'GET'])
def csv_fixed_data():
	print 'request csv_fixed_data' 
	
	# pdb.set_trace();
	
	import csv
	csvdata = request.args['data']

	import StringIO
	reader = csv.reader(StringIO.StringIO(csvdata))

	csv_reader = csv.DictReader(StringIO.StringIO(csvdata))
	json_result = json.dumps([x for x in csv_reader])
	# json_result = json.dumps([x.encode('utf-8') for x in csv_reader])

	return Response(json.dumps(json_result), mimetype='application/json')

@app.route("/<page>")
def index(page=None):
	print 'requesting... %r' % page
	if page == None:
